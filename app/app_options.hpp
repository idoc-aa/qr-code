...
private:
    ...
    template <typename T>
    std::pair<bool, T> getVersion(std::optional<T> var) const
    {
    ...
    bool parse(int argc, char *argv[])
    {
        ...
        if (checkValue(mQrVersion)
            && checkValue(mEccType)
            && checkValue(mQrStr)
            && checkValue(mFileName))
        {
            return true;
        }
        return false;
